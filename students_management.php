...
if (isset($_GET['edit_id'])) {
    ...
    // جلب جميع الصفوف للمرحلة المحددة مسبقاً
    if ($edit_student && $edit_student['stage_id']) {
        ...
        $grades_stmt->execute();
        $edit_student_grades = $grades_stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    
    // جلب الرسوم الحالية للطالب في وضع التعديل
    if ($edit_student) {
        $current_fees_query = "SELECT fee_type_id FROM student_fees WHERE student_id = :student_id AND fee_type_id IS NOT NULL AND area_id IS NULL";
        $current_fees_stmt = $db->prepare($current_fees_query);
        $current_fees_stmt->bindParam(':student_id', $edit_student['id']);
        $current_fees_stmt->execute();
        $current_student_fees = $current_fees_stmt->fetchAll(PDO::FETCH_COLUMN, 0);
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] == 'add_student') {
        try {
            // الحصول على أعلى كود طالب موجود
            ...
            if (empty($full_name) || empty($stage_id) || empty($grade_id) || empty($parent_father_name) || empty($parent_father_phone) || empty($student_code)) {
                ...
                if ($stmt->execute()) {
                    ...
                    // إضافة الرسوم الدراسية للطالب الجديد
    ...
    if ($_POST['action'] == 'edit_student') {
        try {
            ...
            // إدارة رسوم النقل
            $current_transport_fee_query = "SELECT id, paid_amount, area_id FROM student_fees 
                                           WHERE student_id = :student_id AND area_id IS NOT NULL";
            $current_transport_fee_stmt = $db->prepare($current_transport_fee_query);
            $current_transport_fee_stmt->bindParam(':student_id', $student_id);
            $current_transport_fee_stmt->execute();
            $current_transport_fee = $current_transport_fee_stmt->fetch(PDO::FETCH_ASSOC);
            
            // الحالة 1: تم اختيار منطقة سكنية جديدة
            if ($area_id) {
                $area_query = "SELECT transport_price, area_name FROM residential_areas WHERE id = :area_id";
                $area_stmt = $db->prepare($area_query);
                $area_stmt->bindParam(':area_id', $area_id);
                ...
                if ($area) {
                    ...
                    if ($current_transport_fee) {
                        $remaining_amount = $transport_amount - $current_transport_fee['paid_amount'];
                        $status = $remaining_amount <= 0 ? 'paid' : ($current_transport_fee['paid_amount'] > 0 ? 'partial' : 'pending');
                        ...
                        $update_area_fee_stmt->bindParam(':id', $current_transport_fee['id']);
                        
                        if (!$update_area_fee_stmt->execute()) {
                            throw new Exception('فشل في تحديث رسوم النقل');
                        }
                    } else {
                        $insert_area_fee_query = "INSERT INTO student_fees (student_id, fee_type_id, original_amount, discount_amount, final_amount, paid_amount, remaining_amount, status, due_date, academic_year, area_id, notes) 
                                                  VALUES (:student_id, NULL, :amount, 0, :amount, 0, :amount, 'pending', DATE_ADD(CURDATE(), INTERVAL 30 DAY), '2024-2025', :area_id, :notes)";
                        ...
                        if (!$insert_area_fee_stmt->execute()) {
                            throw new Exception('فشل في إضافة رسوم النقل');
                        }
                    }
                }
            } else {
                // الحالة 2: تم إلغاء اختيار المنطقة السكنية
                if ($current_transport_fee) {
                    if ($current_transport_fee['paid_amount'] == 0) {
                        $delete_area_fee_query = "DELETE FROM student_fees WHERE id = :id AND student_id = :student_id";
                        $delete_area_fee_stmt = $db->prepare($delete_area_fee_query);
                        $delete_area_fee_stmt->bindParam(':id', $current_transport_fee['id']);
                        $delete_area_fee_stmt->bindParam(':student_id', $student_id);
                        
                        if (!$delete_area_fee_stmt->execute()) {
                            throw new Exception('فشل في حذف رسوم النقل');
                        }
                    } else {
                        $update_area_fee_query = "UPDATE student_fees SET area_id = NULL, notes = CONCAT(notes, ' (ملغاة)') WHERE id = :id";
                        $update_area_fee_stmt = $db->prepare($update_area_fee_query);
                        $update_area_fee_stmt->bindParam(':id', $current_transport_fee['id']);
                        
                        if (!$update_area_fee_stmt->execute()) {
                            throw new Exception('فشل في تحديث رسوم النقل الملغاة');
                        }
                    }
                }
            }
            ...
            if (isset($_POST['selected_fees_data'])) {
                $selected_fees = json_decode($_POST['selected_fees_data'], true);
                if (!is_array($selected_fees)) {
                    $selected_fees = [];
                }
                
                // جلب الرسوم الحالية للطالب (باستثناء رسوم النقل)
                $current_fees_query = "SELECT sf.id, sf.fee_type_id, sf.paid_amount, ft.is_mandatory, ft.fee_name
                                      FROM student_fees sf 
                                      JOIN fee_types ft ON sf.fee_type_id = ft.id 
                                      WHERE sf.student_id = :student_id AND sf.area_id IS NULL";
                $current_fees_stmt = $db->prepare($current_fees_query);
                $current_fees_stmt->bindParam(':student_id', $student_id);
                $current_fees_stmt->execute();
                $current_fees = $current_fees_stmt->fetchAll(PDO::FETCH_ASSOC);
                
                // إنشاء مصفوفة للرسوم الحالية
                $current_fee_ids = [];
                foreach ($current_fees as $current_fee) {
                    $current_fee_ids[] = $current_fee['fee_type_id'];
                }
                
                // إضافة الرسوم الجديدة المختارة
                foreach ($selected_fees as $fee_id) {
                    if (!in_array($fee_id, $current_fee_ids)) {
                        $fee_query = "SELECT * FROM fee_types WHERE id = :fee_id AND is_active = 1";
                        $fee_stmt = $db->prepare($fee_query);
                        $fee_stmt->bindParam(':fee_id', $fee_id);
                        $fee_stmt->execute();
                        $fee = $fee_stmt->fetch(PDO::FETCH_ASSOC);
                        ...
                        if ($fee) {
                            ...
                            if (!$insert_stmt->execute()) {
                                throw new Exception('فشل في إضافة الرسوم الجديدة');
                            }
                        }
                    }
                }
                ...
                foreach ($current_fees as $current_fee) {
                    if (!in_array($current_fee['fee_type_id'], $selected_fees)) {
                        if ($current_fee['is_mandatory'] == 0 && $current_fee['paid_amount'] == 0) {
                            $delete_query = "DELETE FROM student_fees WHERE id = :id AND student_id = :student_id";
                            $delete_stmt = $db->prepare($delete_query);
                            $delete_stmt->bindParam(':id', $current_fee['id']);
                            $delete_stmt->bindParam(':student_id', $student_id);
                            
                            if (!$delete_stmt->execute()) {
                                throw new Exception('فشل في حذف الرسوم الملغاة');
                            }
                        }
                    }
                }
            ...
            // تأكيد جميع التحديثات
            $db->commit();
            
            $_SESSION['message'] = '<div class="alert alert-success">تم تعديل بيانات الطالب بنجاح مع تحديث جميع الرسوم</div>';
            header("Location: students_management.php");
            exit;
            
        } catch (Exception $e) {
            $db->rollBack();
            $message = '<div class="alert alert-danger">خطأ في تعديل بيانات الطالب: ' . $e->getMessage() . '</div>';
        }
    }
}
...
<head>
    ...
    <style>
        :root {
            ...
            --primary-gradient: linear-gradient(145deg, #3B82F6 0%, #1E40AF 100%);
        ...
        /* ========================================================================= */
        /* القسم الثاني: العناصر الأساسية والخطوط */
        /* ========================================================================= */
        * {
            margin: 0;
            padding: 0;
        ...
        /* ========================================================================= */
        /* القسم الثالث: الشريط الجانبي (Sidebar) - أنيق ومضغوط */
        /* ========================================================================= */
        .sidebar {
            width: 80px;
      ...
      /* زر فتح/إغلاق القائمة */
        ...
        /* Mobile Toggle Button (محذوف لتجنب التكرار، الكود يشتمل عليه) */
        
        /* ========================================================================= */
        /* القسم الرابع: المحتوى الرئيسي (Main Content) */
        /* ========================================================================= */
        .main-content {
            flex: 1;
            margin-right: 80px;
        ...
        .user-details {
            text-align: left;
            padding-left: 10px; /* لإضافة مساحة قبل زر الخروج */
        }
        ...
        .content {
            ...
            display: grid;
            gap: 24px;
            grid-template-columns: 1fr; /* تخطيط شبكي */
        }
...
/* باقي الـ CSS الخاص بصفحة payments يبقى كما هو */
.alert {
    padding: 16px 20px;
    border-radius: var(--border-radius);
    margin-bottom: 24px;
...
@media (min-width: 1200px) {
	...
	.view-section.active, .tab-section.active {
		display: block;
	}
...
.card-body {
    padding: 24px;
      ...
      /* تحسينات للهواتف المحمولة */
        ...
        /* تحسينات إضافية للاستجابة على الشاشات الصغيرة والهواتف */
        @media (max-width: 768px) {
            /* جعل التبويبات في عمود واحد بعرض كامل */
            .view-toggle {
                flex-direction: column;
                align-items: stretch;
                padding: 8px;
            ...
            /* تقليل الحواف الداخلية للكروت */
            .card-header,
            .card-body {
                padding: 14px 16px;
            }

            /* جعل حقول الفورم في عمود واحد لسهولة القراءة */
            .form-grid,
            .form-row,
            .equal-width {
            ...
            /* جعل جداول البيانات قابلة للتمرير أفقياً عند صغر الشاشة */
            .table-responsive,
            ...
            /* تحسين الهيدر العلوي قليلاً */
            .header {
                padding: 10px 12px;
            }
			
			/* ضبط قسم الرسوم على الموبايل لمنع تمدد العرض */
			#feesSelection,
			#mandatoryFees,
			#optionalFees,
			#mandatoryFeesList,
    ...
    <div class="app-container">
        ...
        <div class="sidebar" id="sidebar">            
            ...
            <ul class="sidebar-menu">
                <li><a href="dashboard.php"><i class="fas fa-chart-line"></i><span>لوحة التحكم</span></a></li>
                <li><a href="students_management.php" class="active"><i class="fas fa-user-graduate"></i><span>إدارة الطلاب</span></a></li>
                <li><a href="payments.php"><i class="fas fa-credit-card"></i><span>تسجيل المدفوعات</span></a></li>
                <li><a href="discounts.php"><i class="fa-solid fa-tags"></i><span>تطبيق الخصومات</span></a></li>
            ...
            <div class="content">
                ...
                <div class="dashboard-grid">
					<div class="view-toggle">
						<button type="button" class="toggle-btn active" data-target="students-list-section">
							<i class="fas fa-list"></i>
							قائمة الطلاب المسجلين
						</button>
						<button type="button" class="toggle-btn" data-target="add-student-section">
							<i class="fas fa-plus"></i>
							إضافة طالب جديد
						</button>
					</div>
                    <div id="add-student-section" class="tab-section">
<div class="card">
                    ...
                    <div id="students-list-section" class="tab-section active">
<div class="card">
                        <div class="card-header">
                            <h2 class="card-title">
                                <i class="fas fa-list"></i>
                        ...
                        <div class="card-body">
                            ...
                            <div class="students-cards">
                                ...
                                <div class="student-card">
									...
									<!-- قسم معلومات أولياء الأمور - بنفس تصميم الطالب -->
									<div class="student-details">
										<div class="parents-info-header">
											<i class="fas fa-users"></i>
											معلومات أولياء الأمور
										...
										<div class="detail-row">
											<div class="detail-item">
												...
												<span class="detail-value"><?php echo htmlspecialchars($student['parent_father_name'] ?? $student['parent_name'] ?? ''); ?></span>
											</div>
											<div class="detail-item">
												<span class="detail-label">
												...
												<span class="detail-value"><?php echo htmlspecialchars($student['parent_father_phone'] ?? $student['parent_phone'] ?? ''); ?></span>
											</div>
										</div>
										...
										<div class="detail-row">
											<div class="detail-item">
												...
												<span class="detail-value"><?php echo htmlspecialchars($student['parent_mother_name'] ?? 'غير محدد'); ?></span>
											</div>
											<div class="detail-item">
												<span class="detail-label">
												...
												<span class="detail-value"><?php echo htmlspecialchars($student['parent_mother_phone'] ?? 'غير محدد'); ?></span>
											</div>
										</div>
										...
										<div class="detail-row">
											<div class="detail-item">
												...
												<span class="detail-value"><?php echo htmlspecialchars($student['parent_other_phone']); ?></span>
											</div>
										</div>
										<?php endif; ?>
									</div>
                ...
                <div class="info-card">
					...
					<div class="info-card">
						...
						<div class="info-item">
							...
							<span class="info-value"><?php echo htmlspecialchars($view_student['parent_father_name'] ?? $view_student['parent_name'] ?? ''); ?></span>
						</div>
						...
						<div class="info-item">
							...
							<span class="info-value"><?php echo htmlspecialchars($view_student['parent_father_phone'] ?? $view_student['parent_phone'] ?? ''); ?></span>
						</div>
						...
						<div class="info-item">
							...
							<span class="info-value"><?php echo htmlspecialchars($view_student['parent_mother_name'] ?? 'غير محدد'); ?></span>
						</div>
						...
						<div class="info-item">
							...
							<span class="info-value"><?php echo htmlspecialchars($view_student['parent_mother_phone'] ?? 'غير محدد'); ?></span>
						</div>
						...
						<div class="info-item">
							...
							<span class="info-value"><?php echo htmlspecialchars($view_student['parent_other_phone']); ?></span>
						</div>
						<?php endif; ?>
...
// نظام التبويبات بين إضافة طالب وقائمة الطلاب

function resetStudentFormToAddMode() {
    const studentForm = document.getElementById('studentForm');
    if (!studentForm) return;

    const actionInput = studentForm.querySelector('input[name="action"]');
    const studentIdInput = studentForm.querySelector('input[name="student_id"]');

    // لو مش في وضع التعديل، متعملش حاجة
    if (actionInput && actionInput.value !== 'edit_student') {
        return;
    }

    // تحويل النموذج إلى وضع إضافة طالب جديد
    if (actionInput) {
    ...
    // تفريغ الحقول النصية والرقمية والتواريخ
    const inputs = studentForm.querySelectorAll('input:not([type="hidden"]):not([type="checkbox"]):not([type="radio"])');
    inputs.forEach(input => {
        input.value = '';
    });

    // تفريغ الـ checkbox و الـ radio
    const checkable = studentForm.querySelectorAll('input[type="checkbox"], input[type="radio"]');
    ...
    // تفريغ الـ textarea
    const textareas = studentForm.querySelectorAll('textarea');
    textareas.forEach(area => {
        area.value = '';
    });

    // تحديث عنوان الكارت إلى "إضافة طالب جديد"
    const title = document.querySelector('#add-student-section .card-title');
    if (title) {
        const icon = title.querySelector('i');
        const label = 'إضافة طالب جديد';
        if (icon) {
            icon.classList.remove('fa-edit');
            icon.classList.add('fa-plus');
            const newIcon = icon.cloneNode(true);
            title.textContent = '';
            title.appendChild(newIcon);
    ...
    // تحديث زر الإرسال ليصبح "إضافة الطالب"
    const submitBtn = studentForm.querySelector('.submit-btn');
    if (submitBtn) {
        const icon = submitBtn.querySelector('i');
        const newText = 'إضافة الطالب';
        ...
        if (icon) {
            ...
            submitBtn.appendChild(document.createTextNode(' ' + newText));
        } else {
...
function initTabs() {
    ...
    // هل نحن في وضع تعديل طالب ؟ (edit_id موجود في الرابط)
    const urlParams = new URLSearchParams(window.location.search);
    const urlHasEdit = urlParams.has('edit_id');

    // القسم الافتراضي
    let initialSectionId = 'students-list-section'; // قائمة الطلاب المسجلين

    // لو في تعديل خلي الافتراضي هو تبويب إضافة/تعديل الطالب
    if (urlHasEdit) {
        initialSectionId = 'add-student-section';
    }

    // تفعيل الزر والقسم الابتدائي
    toggleButtons.forEach(btn => {
        const targetId = btn.getAttribute('data-target');
        if (targetId === initialSectionId) {
    ...
    // ربط أحداث الضغط على أزرار التبويبات
    toggleButtons.forEach(button => {
        button.addEventListener('click', function () {
            ...
            // إظهار القسم المختار وإخفاء الباقي
            sections.forEach(sec => {
                if (sec.id === targetId) {
                    sec.classList.add('active');
                } else {
            ...
            // لو ضغطنا على "إضافة طالب جديد" نحول النموذج لوضع الإضافة
            if (targetId === 'add-student-section') {
                if (typeof resetStudentFormToAddMode === 'function') {
                    resetStudentFormToAddMode();
                }

                // إزالة edit_id من الرابط حتى لا يرجع لوضع التعديل بعد إعادة التحميل
                try {
                    ...
                    console.warn('تعذر تحديث الرابط بعد إزالة edit_id', e);
                }
            }
        });
    });
    ...
    toggleButtons.forEach(button => {
        button.addEventListener('click', function () {
            ...
            // حفظ التبويب في التخزين المحلي
            try {
                localStorage.setItem('students_current_section', targetId);
            } catch (e) {
                console.warn('تعذر حفظ حالة التبويب في localStorage', e);
            }
		...
		// في حالة الضغط على تبويب "إضافة طالب جديد" نحول النموذج لوضع الإضافة
		if (targetId === 'add-student-section') {
			resetStudentFormToAddMode();
			...
			try {
				...
				console.warn('تعذر تعديل الرابط لإزالة edit_id', e);
                }
            }
        });
    });
 ...
 // دالة التحقق من الجوال
    ...
    function isSamePageHref(href) {
        ...
        try {
            ...
            return (H.p === L.p) && (H.s === L.s) && (!H.h || H.h === L.h || H.h === '#');
        } catch(e) { 
            return false; 
        }
    }
    ...
    function handleLinkClick(e) {
        ...
        // إذا كان الرابط يحتوي على data-no-close، لا نغلق القائمة
        ...
        // إذا كان نفس الصفحة، فقط أغلق القائمة
        if (!href || href === '#' || href.toLowerCase().startsWith('javascript:') || isSamePageHref(href)) {
            closeSidebar();
            return;
        }
        
        // إذا كان رابط خارجي أو صفحة جديدة
        closeSidebar();
    ...
    // إعدادات للكمبيوتر (فتح القائمة عند التمرير)
    if (!isMobile()) {
    ...
    // إعادة التعيين عند تغيير حجم النافذة
    function handleResize() {
        if (!isMobile()) {
            // إعادة تعيين لسطح المكتب
            document.body.classList.remove('sidebar-open');
            sidebar.classList.remove('open', 'active');
            document.body.style.overflow = '';
            if (overlay) { 
...
// =============================================
// نظام المناطق السكنية الهرمي - الإصدار المحسن
// =============================================
...
function initResidentialAreas() {
    ...
    if (mainAreaSelect) {
        mainAreaSelect.addEventListener('change', function() {
            ...
            console.log('تم اختيار منطقة رئيسية:', parentId);
            
            if (parentId) {
                loadSubAreas(parentId);
            } else {
                resetSubAreaSelection();
                updateTotalFees();
            }
        });
    ...
    if (subAreaSelect) {
        subAreaSelect.addEventListener('change', function() {
            console.log('تم اختيار منطقة فرعية:', this.value);
            updateAreaPrice();
            updateTotalFees();
        });
    }
    ...
    <?php if (isset($edit_student) && $edit_student && $edit_student['area_id']): ?>
...
// إعادة تعيين المناطق الفرعية
function resetSubAreaSelection() {
    ...
    console.log('تم إعادة تعيين المناطق الفرعية');
}
...
// تحميل المناطق الفرعية - الإصدار المحسن
function loadSubAreas(parentId) {
    ...
    xhr.timeout = 5000; // تقليل المهلة إلى 5 ثواني
...
// تحديث سعر المنطقة المختارة - الإصدار المحسن
...
// تحميل بيانات المنطقة في وضع التعديل - الإصدار المحسن
function loadEditAreaData(areaId) {
            ...
            if (xhr.status === 200) {
                try {
                    ...
                    if (areaData.success && areaData.parent_id) {
                        initializeEditAreaImmediately(areaData.parent_id, areaId);
                    } else {
    ...
    xhr.ontimeout = function() {
        console.error('انتهت مهلة تحميل بيانات المنطقة');
    };
...
// تهيئة المنطقة فوراً بدون تأخير
function initializeEditAreaImmediately(parentId, areaId) {
    console.log('تهيئة المنطقة فوراً - الرئيسية:', parentId, 'الفرعية:', areaId);
    ...
    if (mainAreaSelect) {
        ...
        // تحميل المناطق الفرعية فوراً
        loadSubAreasImmediately(parentId, areaId);
    }
}

// تحميل المناطق الفرعية فوراً مع التحديد
function loadSubAreasImmediately(parentId, targetAreaId) {
    const subAreaGroup = document.getElementById('studentSubAreaGroup');
    const subAreaSelect = document.getElementById('studentSubAreaSelect');
    
    console.log('تحميل المناطق الفرعية فوراً للمنطقة الرئيسية:', parentId);
        ...
        if (xhr.readyState === 4) {
            ...
            if (xhr.status === 200) {
                try {
                    const subAreas = JSON.parse(xhr.responseText);
                    console.log('المناطق الفرعية التي تم جلبها:', subAreas);
                    populateAndSelectSubArea(subAreas, targetAreaId);
                } catch (e) {
                    console.error('خطأ في تحليل البيانات:', e);
                    showErrorState(subAreaSelect, 'خطأ في تنسيق البيانات');
                }
...
// تعبئة وتحديد المنطقة الفرعية فوراً
function populateAndSelectSubArea(subAreas, targetAreaId) {
    const subAreaSelect = document.getElementById('studentSubAreaSelect');
    const subAreaGroup = document.getElementById('studentSubAreaGroup');
    ...
    if (subAreas.length > 0) {
        ...
        subAreas.forEach(area => {
            ...
            // التحقق إذا كانت هذه هي المنطقة المستهدفة
            if (area.id == targetAreaId) {
                subAreaSelect.value = area.id;
                targetFound = true;
                console.log('تم تحديد المنطقة الفرعية فوراً:', area.id);
            }
        });
        ...
        // إذا تم العثور على المنطقة المستهدفة، تحديث السعر فوراً
        if (targetFound) {
            updateAreaPrice();
        }
    } else {
...
// =============================================
// نظام الرسوم الدراسية - الإصدار المحسن
// =============================================

function createFeeElement(fee, isChecked = true) {
    ...
    div.innerHTML = `
        <input type="checkbox" name="selected_fees[]" value="${fee.id}" 
               ${isMandatory ? 'checked disabled' : (isChecked ? 'checked' : '')}
    ...
    // إضافة حدث تغيير للرسوم الاختيارية
    if (!isMandatory) {
...
function displayFees(fees, currentFees = []) {
    ...
    console.log('=== عرض الرسوم ===');
    console.log('الرسوم المتاحة:', fees);
    console.log('الرسوم الحالية للطالب:', currentFees);
    
    // فصل الرسوم الإجبارية والاختيارية
    fees.forEach(fee => {
        // تحديد إذا كان الرسم مختار مسبقاً
        let isChecked = false;
        
        if (fee.is_mandatory == 1) {
            // الرسوم الإجبارية دائماً مختارة
            isChecked = true;
        } else {
            // الرسوم الاختيارية: نتحقق إذا كانت موجودة في الرسوم الحالية للطالب
            isChecked = currentFees.includes(parseInt(fee.id));
        }
        
        console.log(`الرسم ${fee.id} - ${fee.fee_name}: ${isChecked ? 'مختار' : 'غير مختار'}`);
        
        const feeElement = createFeeElement(fee, isChecked);
        
        if (fee.is_mandatory == 1) {
            mandatoryFeesList.appendChild(feeElement);
            hasMandatory = true;
            // الرسوم الإجبارية دائماً مضاف لها المبلغ
            totalAmount += parseFloat(fee.amount);
        } else {
            optionalFeesList.appendChild(feeElement);
            hasOptional = true;
            // الرسوم الاختيارية تضاف فقط إذا كانت مختارة
            if (isChecked) {
                totalAmount += parseFloat(fee.amount);
            }
        }
    ...
    // تحديث العرض فوراً
    updateFeesDisplayImmediately(hasMandatory, hasOptional, totalAmount);
}

function updateFeesDisplayImmediately(hasMandatory, hasOptional, totalAmount) {
    ...
    if (totalElement) {
        if (totalAmount > 0) {
            ...
            totalElement.innerHTML = `<i class="fas fa-calculator"></i> إجمالي الرسوم المختارة: 0.00 <?php echo CURRENCY; ?>`;
            totalElement.style.display = 'block';
        }
    }
...
// تحميل الرسوم الدراسية - الإصدار المحسن
function loadFees(stageId, gradeId) {
    if (!stageId || !gradeId) {
        console.log('بيانات غير كافية لتحميل الرسوم');
        resetFeesSelection();
        return;
    }
    
    console.log('بدء تحميل الرسوم للمرحلة:', stageId, 'والصف:', gradeId);
        ...
        if (xhr.readyState === 4) {
            if (xhr.status === 200) {
                try {
                    ...
                    console.log('تم تحميل الرسوم للمرحلة', stageId, 'والصف', gradeId, ':', fees);
                    
                    // تحديد الرسوم الحالية بناءً على وضع التعديل أو الإضافة
                    let currentFees = [];
                    
                    <?php if (isset($edit_student) && $edit_student && !empty($current_student_fees)): ?>
                    // في وضع التعديل: نستخدم الرسوم الحالية من قاعدة البيانات
                    currentFees = <?php echo json_encode($current_student_fees); ?>;
                    console.log('وضع التعديل - استخدام الرسوم الحالية:', currentFees);
                    <?php else: ?>
                    // في وضع الإضافة: جميع الرسوم الإجبارية تكون مختارة
                    currentFees = fees.filter(fee => fee.is_mandatory == 1).map(fee => parseInt(fee.id));
                    console.log('وضع الإضافة - الرسوم الإجبارية مختارة:', currentFees);
                    <?php endif; ?>
                    
                    displayFeesImmediately(fees, currentFees);
                } catch (e) {
                    console.error('خطأ في تحميل الرسوم:', e);
                    showFeesError();
                }
            } else {
                console.error('خطأ في استجابة الخادم:', xhr.status);
                showFeesError();
            }
        }
    };
    ...
    xhr.ontimeout = function() {
        console.error('انتهت مهلة تحميل الرسوم');
        showFeesError();
    };
    
    xhr.onerror = function() {
        console.error('خطأ في الاتصال لتحميل الرسوم');
        showFeesError();
    };
...
// عرض الرسوم فوراً بدون تأخير
function displayFeesImmediately(fees, currentFees = []) {
    const mandatoryFeesList = document.getElementById('mandatoryFeesList');
    const optionalFeesList = document.getElementById('optionalFeesList');
    const feesSelection = document.getElementById('feesSelection');
...
function initFeesSystem() {
    ...
    if (gradeSelect) {
        gradeSelect.addEventListener('change', function() {
            ...
            console.log('تم تغيير الصف - المرحلة:', stageId, 'الصف:', gradeId);
            
            if (stageId && gradeId) {
                loadFees(stageId, gradeId);
            } else {
                hideFeesSelection();
                console.log('إخفاء قسم الرسوم - بيانات غير مكتملة');
            }
        });
    }
    
    // تحميل الرسوم مباشرة إذا كانت هناك قيم افتراضية (في وضع التعديل)
    <?php if (isset($edit_student) && $edit_student && $edit_student['stage_id'] && $edit_student['grade_id']): ?>
    setTimeout(() => {
        const stageId = <?php echo $edit_student['stage_id']; ?>;
        const gradeId = <?php echo $edit_student['grade_id']; ?>;
        console.log('تحميل الرسوم الافتراضية للتعديل:', stageId, gradeId);
        loadFees(stageId, gradeId);
    }, 100);
    <?php endif; ?>
...
function updateTotalFees() {
    ...
    console.log('إجمالي الرسوم المحدث:', totalAmount);
}
...
function loadInitialData() {
    ...
    // حل فوري لمشكلة الصف الدراسي - بدون تأخير
    <?php if (isset($edit_student) && $edit_student): ?>
    console.log('=== تحميل فوري للبيانات في وضع التعديل ===');
    console.log('معرف الطالب:', <?php echo $edit_student['id']; ?>);
    console.log('المرحلة:', <?php echo $edit_student['stage_id']; ?>);
    console.log('الصف:', <?php echo $edit_student['grade_id']; ?>);
    console.log('الرسوم الحالية:', <?php echo json_encode($current_student_fees); ?>);
    ...
    // تحميل المناطق السكنية فوراً
    <?php if ($edit_student['area_id']): ?>
    console.log('تحميل المنطقة السكنية فوراً:', <?php echo $edit_student['area_id']; ?>);
    loadEditAreaData(<?php echo $edit_student['area_id']; ?>);
    <?php endif; ?>
    <?php endif; ?>
    
    // إذا كانت الصفحة تحتوي على طالب للعرض
    <?php if (isset($view_student) && $view_student): ?>
...
// تقليل استخدام setTimeout
function optimizePerformance() {
    // تعطيل التأخيرات غير الضرورية
    console.log('تفعيل تحسينات الأداء...');
    
    // إضافة مؤشر تحميل سريع
    const loadingIndicator = document.createElement('div');
    loadingIndicator.id = 'quickLoading';
...
// استدعاء تحسينات الأداء عند تحميل الصفحة
document.addEventListener('DOMContentLoaded', function() {
    optimizePerformance();
});
